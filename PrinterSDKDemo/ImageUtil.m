...
+ (NSData *)ditherImage:(UIImage *)image {
  ...
  // 获取灰度数据的长度，即像素数量
  NSUInteger pixelCount = grayscaleData.length;
  
  // 计算二值位图数据的长度，每8个像素压缩成1个字节
  NSUInteger bitmapLength = (pixelCount + 7) / 8; // 确保能够包含所有像素
  NSMutableData *binaryBitmapData = [NSMutableData dataWithLength:bitmapLength];
  unsigned char *bitmapBytes = [binaryBitmapData mutableBytes];
  
  // 初始化数据为0
  memset(bitmapBytes, 0, bitmapLength);
  
  // 遍历灰度数据中的每个像素，应用阈值化，并且每8个像素设置一个字节
  for (NSUInteger i = 0; i < pixelCount; i++) {
    // 如果灰度值高于阈值，则设置相应的位为1
    if (grayscaleDataPtr[i] < 122) { // 使用255代表白色
      NSUInteger byteIndex = i / 8;
      NSUInteger bitIndex = 7 - (i % 8); // 位序从左到右（0为最高位）
